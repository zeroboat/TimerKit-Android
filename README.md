...
TimerKit은 Android 전용 멀티 타이머 애플리케이션입니다. 타바타, 러닝, 멀티 타이머 3가지 핵심 기능을 하나의 앱에서 제공하며, GPS 경로 기록 및 히스토리, Google Maps 연동, 심박수 측정(Health Connect), 음악 컨트롤을 지원합니다.
...
- **러닝 히스토리**: 완료된 러닝 기록을 Room DB에 저장, 날짜/거리/시간/페이스/심박수 목록 및 상세 지도 조회
- **Health Connect 연동**: Galaxy Watch 등 Wear OS 워치 심박수 실시간 표시, 결과 화면에 평균/최대 BPM 표시
  - 심박수 미수신 시 `?` 버튼으로 연결 문제 해결 안내 다이얼로그 제공
...
- **저장**: DataStore Preferences (타이머 설정 영속 저장), Room DB (러닝 히스토리)
- **DB 코드 생성**: KSP (Kotlin Symbol Processing)
...
│   ├── RunningRecord.kt         # Room Entity
│   ├── RunningRecordDao.kt      # DAO (CRUD + 통계 쿼리)
│   ├── RunningRepository.kt     # 저장/조회/JSON 파싱
│   ├── RunningHistoryViewModel.kt
│   ├── RunningHistoryScreen.kt  # 히스토리 목록
│   └── RunningHistoryDetailScreen.kt  # 상세 지도 + km 기록
└── cooking/
    ├── CookingViewModel.kt
    └── CookingScreen.kt         # 멀티 타이머 (동시 실행)
```
